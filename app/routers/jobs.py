...
async def read_jobs(
    ...
    """
    Retrieve job listings created by the current user.
    Requires authenticated user.
    """
    logger.info(f"User {current_user.uid} fetching jobs (skip={skip}, limit={limit})")
    
    # Update query to select new columns
    query = """
        SELECT id, title, description, location, location_type, seniority_level, created_by_user_id, created_at 
        FROM jobs 
    ...
    try:
            ...
            with conn.cursor(cursor_factory=DictCursor) as cur:
                ...
                jobs_list = [
                    JobRead(
                        ...
                        location_type=row['location_type'], # Add new field
                        seniority_level=row['seniority_level'], # Add new field
        ...
        logger.error(f"Unexpected error reading jobs for user {current_user.uid}: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
